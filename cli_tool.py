...
def generate_timeline(logdir):
    ...
    if timeline_entries:
        with open("d3 timeline.csv", "w", newline="", encoding="utf-8") as file:
    ...
    with open("d3 invalid_logs.csv", "w", newline="", encoding="utf-8") as file:
...
def save_anomalies(anomalies):
    with open("d4 flaggedanomalies.csv", "w", newline='') as csvfile:
...
def generate_visuals():
    timeline_df = pd.read_csv("d3 timeline.csv")
    anomalies_df = pd.read_csv("d4 flaggedanomalies.csv")
...
def main():
    ...
    parser.add_argument("logdir", nargs="?", default=r"C:\Users\chitt\Desktop\log_files",
    ...
    if args.summary:
        df = pd.read_csv("d3 timeline.csv")
        print("\n=== SUMMARY ===")
